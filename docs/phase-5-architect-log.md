...
2026-07-05: 5C NON-OBJECTION (architect, staged-diff shape gate).
Verified against D60/D26/D56/D59 + spec §5C on the staged SettingsForm/
SettingsWindowController diff: Dictionary grouped section after App
Categories inside the one Form; rows in list order (order = D59
priority), remove = exact-match removeAll (safe — draft seeds from the
D56-normalized store, exact duplicates impossible); add = trim +
empty/dup no-op + append-to-end; draft plumbing is the D26 triple
(@Published / reload seed / save write, sole persist via draft.save());
no reorder surface; 700 pt at the single R40 site, comment updated.
R53: D60 row AMENDED — clear-on-successful-add-only is the ruling;
