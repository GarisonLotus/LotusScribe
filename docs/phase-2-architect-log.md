...
| D35 | 2026-07-05 | Perceptual waveform mapping (human-gate: bars visually frozen; speech RMS ~0.02–0.15 pins linear bars near floor): pure `AudioLevel.display(rms:) -> Float` — dBFS clamped [-50, 0] → 0…1, exact 0 stays 0 — applied at render time in `PillView.barHeight`. D32 unamended: stored/plumbed levels stay raw 0…1 RMS | Perceptual curve is presentation-only, so it lives at the last presentation step; raw RMS preserved for future consumers (e.g. VAD); D14 holds — map is pure/headless, view stays thin | 2C |
