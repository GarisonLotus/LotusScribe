...
| D118 | 2026-07-08 | `MicrophonePicker` (11D) shows an in-menu `checkmark` on the active device row (and the System Default row when following), reading the shared model's `isChecked`/`defaultIsChecked`. This DIVERGES from `HotkeyPicker` (which marks the selection on the capsule label only, no in-menu checkmark) but MATCHES the 11C NSMenu submenu's `.state`. Kept. | The idiom that matters here is cross-surface consistency between the two MIC surfaces (11C submenu ↔ 11D picker) — both render the same `AudioInputMenuModel`, so both show the checkmark; D114's rationale ("keeps both surfaces' checkmarks in sync") names exactly this. HotkeyPicker is a different control shape (12 fixed F-keys + a custom field, selection obvious from the capsule/field), whereas a list of N similar device names genuinely benefits from an in-menu mark for selection clarity, and the checkmark is SwiftUI's native selection idiom for a Menu of options. Copy/idiom lock, not a redesign. | 11D |
