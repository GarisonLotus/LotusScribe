...
| R74 | 8C | NOTE-ONLY, log semantics (spec check ran): the `AX denylist` log line fires for ANY denylisted-bundle insertion, including one where the settable probe ALSO failed (D61 alone would have picked pasteboard) — attribution reads "denylist-forced" when the denylist merely agreed. Judged harmless-to-desirable: route is identical either way, and the 8C live verify (spec step 2) wants exactly this line whenever Slack is the target, independent of what Slack's probe answers that day. No code action | note |
| R75 | 8C | The adapter's log branch re-derives denylist membership (`axDenylist.contains`) instead of the policy returning a route reason — 1-line duplication, correct today because membership → `.pasteboard` unconditionally. WATCH: if `route()` ever gains a second pasteboard-forcing input, the log arm's membership check could mislabel the reason; at that point make the policy return (route, reason) rather than adding a third adapter-side re-check | open (watch) |
