...
> Waveform-display fix reviewed 2026-07-05: EXECUTION APPROVED. dB map
> verified by hand: `display(rms:)` guard `rms > 0` catches 0, negatives,
> AND NaN (NaN > 0 is false) — no log10-of-nonpositive path; rms > 1
> (R30a's ≈1.00003) yields tiny positive dB, clamped to 1 by the `min`;
> -26 dBFS speech (0.05) → 0.48, mid-range as claimed; monotonic inside
> the window since log10 is. Tests are range/order-based, no
> magic-decimal equality (the one `== 1.0` is exact by construction:
> log10(1) = 0). Display-only invariant holds: mapping lives solely in
> PillView.barHeight at render time; PillViewModel storage and the D32
> raw-RMS pipeline untouched. Log line sits inside the `!engineLive`
> flip and engineLive resets only in startRecording — one-shot per
> capture, no per-level spam. Independent `make test`: 80 tests /
> 12 suites green. No new rows; R30/R32 annotated below.
...
| R32 | 2B | PillView bar-geometry literals (bar width 3, spacing 3, 4 pt height floor, 24 pt interior inset in `barHeight`) are view-local, single-site each — D31 constants themselves have no second site, so no violation. But the interior-inset `24` numerically coincides with `PillMetrics.bottomMargin`; name it or comment it if it ever wants a second site (R21 trigger) | open (non-blocking) — waveform-fix note (2026-07-05): diff touched barHeight but the literals (4 pt floor, 24 interior inset) persist unchanged and remain single-site; status unchanged in substance. The dB window bounds (-50, 20, 50) in AudioLevel.display are likewise view-of-one literals, documented in the doc comment — same non-blocking posture |
