...
2026-07-05: 4C NON-OBJECTION (architect gate). Read the staged
SettingsForm.swift diff against D53/D54: App Categories grouped-Form
section with one row per override (middle-truncated bundle-ID text +
displayName Picker + remove), all mutations draft-dict-only (D26/D53 —
sole persist via draft.save(), reload reseeds); "Add App…" menu of
running .regular apps seeding from the built-in map with a
duplicate-add no-op guard (a re-add cannot clobber an edited row);
categoryBinding get mirrors D53 resolution so a garbage stored value
displays as the built-in fallback but is rewritten only on an actual
pick (set-only write, reviewer round-trip pinned); contentSize
390→560 at the single R40 site. Reviewer PASS (sequential,
independent 149/17) accepted; R44 note-only (safe leg); R45 accepted
and the D53 row wording amended above — the invariant is "overrides
never trigger a probe or warm-up," not "overrides-only save fires no
